...
namespace YHExcelAddin.Calculator.Calculator
{
	/// <summary>
		...
		public string[] FindUnKnown(string []NotInclude)
		{
			...
			for (int i = 0; i < str.Length; i++) {
				if (tpm=="") {
					...
					{
						if (tpm!="") {
							if(CheckParamerIn(re,tpm)&&!(new Calculator()).CheckFuncName(tpm)&&CheckParamerIn(NotInclude,tpm))
				{
								re[j]=tpm;
					         j++;}
							tpm="";
		...
		public string[] FindUnKnown( )
		{
			...
			for (int i = 0; i < str.Length; i++) {
				if (tpm=="") {
					...
					{
						if (tpm!="") {
							if(CheckParamerIn(re,tpm)&&!(new Calculator()).CheckFuncName(tpm))
				{
								re[j]=tpm;
					         j++;}
							tpm="";
